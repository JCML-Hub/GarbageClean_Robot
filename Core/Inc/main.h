/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2024 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f1xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define LED1_Pin GPIO_PIN_13
#define LED1_GPIO_Port GPIOC
#define LED2_Pin GPIO_PIN_14
#define LED2_GPIO_Port GPIOC
#define LED3_Pin GPIO_PIN_15
#define LED3_GPIO_Port GPIOC
#define Key_Pin GPIO_PIN_1
#define Key_GPIO_Port GPIOA
#define USR_BOOT1_Pin GPIO_PIN_4
#define USR_BOOT1_GPIO_Port GPIOA
#define USR_BOOT2_Pin GPIO_PIN_5
#define USR_BOOT2_GPIO_Port GPIOA
#define END1_Pin GPIO_PIN_6
#define END1_GPIO_Port GPIOA
#define END2_Pin GPIO_PIN_7
#define END2_GPIO_Port GPIOA
#define END3_Pin GPIO_PIN_0
#define END3_GPIO_Port GPIOB
#define END4_Pin GPIO_PIN_1
#define END4_GPIO_Port GPIOB
#define CHRG_Pin GPIO_PIN_2
#define CHRG_GPIO_Port GPIOB
#define Echo1_Pin GPIO_PIN_12
#define Echo1_GPIO_Port GPIOB
#define Echo2_Pin GPIO_PIN_13
#define Echo2_GPIO_Port GPIOB
#define Echo3_Pin GPIO_PIN_14
#define Echo3_GPIO_Port GPIOB
#define Echo4_Pin GPIO_PIN_15
#define Echo4_GPIO_Port GPIOB
#define PWM1_Pin GPIO_PIN_8
#define PWM1_GPIO_Port GPIOA
#define PWM2_Pin GPIO_PIN_11
#define PWM2_GPIO_Port GPIOA
#define BUZZER_Pin GPIO_PIN_12
#define BUZZER_GPIO_Port GPIOA
#define M1CCW_Pin GPIO_PIN_15
#define M1CCW_GPIO_Port GPIOA
#define M2CCW_Pin GPIO_PIN_3
#define M2CCW_GPIO_Port GPIOB
#define M3CCW_Pin GPIO_PIN_4
#define M3CCW_GPIO_Port GPIOB
#define M4CCW_Pin GPIO_PIN_5
#define M4CCW_GPIO_Port GPIOB
#define PWM3_Pin GPIO_PIN_6
#define PWM3_GPIO_Port GPIOB
#define PWM4_Pin GPIO_PIN_7
#define PWM4_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
